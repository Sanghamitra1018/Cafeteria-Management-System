...
from django.core.mail import send_mail      # to send OTP to mail 
...
def check_out(request):
    if not request.session.get("OTP"):      # if otp is not generated at all then this is executed
        ...
        send_mail(                          # send_mail is imported
            "OTP from SIT Cafe",         # subject of email
            f"Your OTP to order food from SIT Cafe is {otp}",
            "sudhasusekharsethy21@gmail.com",        # two step verification turned ON email, this is the sender email
...
def place_order(request):
    if request.method == "POST":
        otp = request.POST.get("otp")       # "otp" is the id of the input field and we are fetching the otp entered by user and storing in local varible otp

        if request.session.get("OTP") != int(otp):      # type cast the otp to integer as the input filed will give string type and match it with the otp generated during the session
